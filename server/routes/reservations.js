...
import { getScope, requireScope } from '../utils/scope.js';
...
router.get('/', asyncRoute(async (req, res) => {
  ...
  for await (const order of client.reservationOrder.list()) {
    ...
    } catch { /* some orders may 403 on line items — skip gracefully */ }
...
router.get('/usage', asyncRoute(async (req, res) => {
  ...
  const recent7DayAvg = Object.entries(
    recentUsage.reduce((acc, item) => {
  ...
  ).map(([key, v]) => ({ key, avgUtilization7d: Math.round((v.sum / v.count) * 10) / 10 }));
...
router.get('/savings', asyncRoute(async (req, res) => {
  const scope = requireScope(req, res);
  if (!scope) return;
  ...
  let totalPaygCost = 0, totalAmortizedCost = 0, totalUsedHours = 0;
  const byOrder = {};
  ...
  for await (const item of client.reservationsSummaries.list(scope, 'monthly', { $filter: filter })) {
    ...
    if (!byOrder[key]) {
      byOrder[key] = { reservationOrderId: key, skuName: item.skuName, paygEquivalent: 0, amortizedCost: 0, saving: 0, usedHours: 0, utilizationPct: 0 };
    }
  ...
  const byOrderArr = Object.values(byOrder)
    .map(o => ({ ...o, savingPct: o.paygEquivalent > 0 ? Math.round((o.saving / o.paygEquivalent) * 100) : 0 }))
...
router.get('/expiring', asyncRoute(async (req, res) => {
  const daysThreshold = Math.min(Math.max(parseInt(req.query.days || '90', 10), 1), 365);
  ...
  for await (const order of client.reservationOrder.list()) {
    ...
    if (daysLeft > 0 && daysLeft <= daysThreshold) {
      expiring.push({
        ...
        action:         daysLeft <= 30
          ? 'Renew immediately to avoid PAYG billing gap'
          : 'Plan renewal within 30 days',
      });
    }
  }
...
router.get('/underutilized', asyncRoute(async (req, res) => {
  const scope = requireScope(req, res);
  if (!scope) return;

  const threshold = Math.min(Math.max(parseFloat(req.query.threshold || '80'), 0), 100);
  ...
  for await (const item of client.reservationsSummaries.list(scope, 'monthly', { $filter: filter })) {
    if ((item.avgUtilizationPercentage || 0) < threshold) {
      ...
      summaries.push({
        ...
        estimatedWastedCost: Math.round((item.onDemandRate || 0) * unusedHours * 100) / 100,
